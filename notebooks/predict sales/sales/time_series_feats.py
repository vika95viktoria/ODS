...
def get_avg_metric_for_block(grouped, gr_local_cols, grouping, lag, date_block_num):
    ...
    gr_local.columns = [f'{grouping}_avg_item_cnt_lag_{lag}', f'{grouping}_max_item_cnt_lag_{lag}', f'{grouping}_min_item_cnt_lag_{lag}']
...
def get_lag_metric(df, group_cols, date_block, lags):
    ...
    lag_columns = [col for lag in lags[1:]
                   for col in [f'{grouping}_avg_item_cnt_lag_{lag}', f'{grouping}_max_item_cnt_lag_{lag}',
                               f'{grouping}_min_item_cnt_lag_{lag}']]
    columns = gr_local_cols + [f'{grouping}_avg_item_cnt_lag_{lags[0]}', f'{grouping}_max_item_cnt_lag_{lags[0]}',
                               f'{grouping}_min_item_cnt_lag_{lags[0]}', 'date_block_num'] + lag_columns
    return pd.DataFrame(final_result, columns=columns)


#grouped = pd.read_csv('grouped_by_item.csv')
# df = pd.read_csv('full_df.csv',  skiprows= 0, nrows=1000000)
# df = df[df.date_block_num < 26]
# res = get_lag_metric(df, ['date_block_num', 'item_id', 'shop_id'], lags=[3, 6, 9])
# print(res.head(10))
...
def calculate_and_add_lag(df, group_cols, date_block=None):
    ...
    for c in group_cols:
        if column_dict[c] == np.int64:
            group_by_cols[c] = group_by_cols[c].astype(np.int64)
    return group_by_cols, group_cols
    # df = pd.merge(df, group_by_cols, on=group_cols, how='left')
    # return df

# full_df = pd.read_csv('real_train.csv')
